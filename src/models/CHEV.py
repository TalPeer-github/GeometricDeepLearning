...
class ChebNet(torch.nn.Module):
    ...
    How it works:
        Spectral convolution: a graph filter is applied to the graph Laplacian's eigenvectors (direct computation of the eigenvectors is costly for large graphs).
        Chebyshev approximation:  approximates the spectral convolution by using Chebyshev polynomials. These polynomials are used to approximate the graph convolution operation to reduce computational cost.
        Chebyshev filter: The convolution operation is approximated using a truncated expansion of Chebyshev polynomials of the graph Laplacian. K - polynomial order, determines how many terms of the expansion are used.
    """
